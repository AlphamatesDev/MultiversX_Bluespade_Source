...
import polygonIcon from "img/ic_polygon_24.svg";
import cronosIcon from "img/ic_cronos_24.svg";
import skaleIcon from "img/ic_skale_24.svg";
      ...
      {props.small ? (
        <div className="App-header-network" onClick={() => setActiveModal(NETWORK_MODAL_KEY)}>
          <div className="network-dropdown">
            <NavIcons selectorLabel={props.selectorLabel} mode={props.mode} />
          </div>
        </div>
      ) : (
        <DesktopDropdown
...
function NavIcons({ selectorLabel, mode }) {
  return (
    <>
      <button className={cx("btn-primary small transparent")}>
        <img
          className="network-dropdown-icon"
          src={selectorLabel === "Cronos" ? cronosIcon : (selectorLabel === "Polygon" ? polygonIcon : skaleIcon)}
          alt={selectorLabel}
        />
      </button>
      <div className="network-dropdown-seperator" />
      <button className={cx("btn-primary small transparent")}>
        <HiDotsVertical color={mode = 'dark' ? "white" : 'black'} size={20} />
      </button>
    </>
  );
}

function DesktopDropdown({ setActiveModal, selectorLabel, networkOptions, onNetworkSelect, openSettings, mode }) {
  return (
    <div className="App-header-network">
      <Menu>
        <Menu.Button as="div" className="network-dropdown">
          <NavIcons selectorLabel={selectorLabel} mode={mode} />
        </Menu.Button>
