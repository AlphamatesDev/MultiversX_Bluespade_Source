...
import { getToken, getTokens, getWhitelistedTokens, getCollateralToken } from "config/tokens";
...
export default function GlpSwap(props) {
  ...
  const tabLabel = isBuying ? t`Buy BLP` : t`Sell BLP`;
  const { active, library, account } = useWeb3React();
  const { chainId } = useChainId();
  // const chainName = getChainName(chainId)
  const tokens = getTokens(chainId);
  const whitelistedTokens = getWhitelistedTokens(chainId);
  const tokenList = whitelistedTokens.filter((t) => t.isCollateralToken);
  ...
  //   chainId === SKALE ? getContract(chainId, "NATIVE_TOKEN") : AddressZero
  // );
  const [swapTokenAddress, setSwapTokenAddress] = useState(tokenList[0].address);
  ...
  const tokenAllowanceAddress = swapTokenAddress;
  ...
  const glpPrice = aum && aum.gt(0) && glpSupply.gt(0)
  ...
  const getError = () => {
    if (IS_NETWORK_DISABLED[chainId]) {
      if (isBuying) return [t`BLP buy disabled, pending ${getChainName(chainId)} upgrade`];
      return [t`BLP sell disabled, pending ${getChainName(chainId)} upgrade`];
    }
    ...
    if (!isBuying) {
      if (maxSellAmount && glpAmount && glpAmount.gt(maxSellAmount)) {
        return [t`Insufficient BLP balance`];
      }
  ...
  const getPrimaryText = () => {
    ...
    return isBuying ? t`Buy BLP` : t`Sell BLP`;
  };
  ...
  const buyGlp = () => {
    ...
    callContract(chainId, contract, method, params, {
      ...
      successMsg: t`${formatAmount(glpAmount, 18, 4, true)} BLP bought with ${formatAmount(
        swapAmount,
  ...
  const sellGlp = () => {
    ...
    callContract(chainId, contract, method, params, {
      ...
      successMsg: t`${formatAmount(glpAmount, 18, 4, true)} BLP sold for ${formatAmount(
        swapAmount,
        swapTokenInfo.decimals,
        4,
        true
    ...
    <div className="GlpSwap">
      ...
      {/* <div className="Page-title-section">
        ...
        {isBuying && <div className="Page-description">
          Purchase <a href="https://bluespadexyz.gitbook.io/bluespade/" target="_blank" rel="noopener noreferrer">GLP tokens</a> to earn {nativeTokenSymbol} fees from swaps and leverage trading.<br/>
        ...
        <div className="App-card GlpSwap-stats-card">
          ...
          <div className="App-card-content">
            ...
            <div className="App-card-row">
              ...
              <div className="value">
                {formatAmount(glpBalance, GLP_DECIMALS, 4, true)} BLP ($
          ...
          <div className="App-card-content">
              ...
              <div className="App-card-row">
                ...
                <div className="value">
                  <Tooltip
                    ...
                    renderContent={() =>
                      t`${formatAmount(reservedAmount, 18, 4, true)} BLP have been reserved for vesting.`
                    }
                  />
                </div>
              </div>
                ...
                <Tooltip
                  ...
                  renderContent={() => {
                    return (
                      <>
                        <StatsTooltipRow
                          label={t`${collateralTokenSymbol} (${collateralTokenSymbol}) APR`}
                          value={`${formatAmount(feeGlpTrackerApr, 2, 2, false)}%`}
                          showDollar={false}
                        />
                        <StatsTooltipRow
                          label={t`Escrowed BLU APR`}
            ...
            <div className="App-card-row">
              ...
              <div className="value">
                <Trans>
                  {formatAmount(glpSupply, GLP_DECIMALS, 4, true)} BLP ($
        ...
        <div className="GlpSwap-box App-box">
          ...
          {isBuying && (
            <BuyInputSection
              ...
              tokenBalance={`${formatAmount(swapTokenBalance, swapToken.decimals, 0, true)}`}
          ...
          {!isBuying && (
            ...
            >
              <div className="selected-token">
                BLP <img src={glp24Icon} alt="glp24Icon" />
              </div>
            </BuyInputSection>
          )}
          ...
          {isBuying && (
            <BuyInputSection
              ...
              topRightLabel={t`Balance:`}
              tokenBalance={`${formatAmount(glpBalance, GLP_DECIMALS, 0, true)}`}
              inputValue={glpValue}
              onInputValueChange={onGlpValueChange}
      ...
      <div className="Tab-title-section">
        ...
        {isBuying && (
          <div className="Page-description">
            <Trans>
              Fees may vary depending on which asset you use to buy BLP. <br />
              Enter the amount of BLP you want to purchase in the order form, then check here to compare fees.
            </Trans>
          </div>
        )}
        {!isBuying && (
          <div className="Page-description">
            <Trans>
              Fees may vary depending on which asset you sell BLP for. <br />
              Enter the amount of BLP you want to redeem in the order form, then check here to compare fees.
            </Trans>
          </div>
        )}
      </div>
                  ...
                  <Tooltip
                    ...
                    renderContent={() => (
                      <p className="text-white">
                        <Trans>Available amount to deposit into BLP.</Trans>
                      </p>
                    )}
                  />
                ) : (
                  <Tooltip
                    ...
                    renderContent={() => {
                      return (
                        <p className="text-white">
                          <Trans>
                            Available amount to withdraw from BLP. Funds not utilized by current open positions.
                          </Trans>
                        </p>
                      );
                    }}
                ...
                case (isBuying && isCapReached) || (!isBuying && managedUsd?.lt(1)):
                  return (
                    <Tooltip
                      ...
                      renderContent={() => (
                        <Trans>
                          Max pool capacity reached for {tokenInfo.symbol}. Please mint BLP using another token
                        </Trans>
                      )}
                    />
                  );
