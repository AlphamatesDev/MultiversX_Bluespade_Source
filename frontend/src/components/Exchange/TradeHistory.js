...
import { useLiquidationsData, useAllTradesHistory } from "domain/legacy";
...
function getPositionDisplay(increase, indexToken, isLong, sizeDelta) {
  const symbol = indexToken ? indexToken.symbol : "";
  return `
...
function renderLiquidationTooltip(liquidationData, label) {
  ...
  const text =
    liquidationData.type === "full"
      ? t`This position was liquidated as the max leverage of 1250x was exceeded.`
      : t`Max leverage of 1250x was exceeded, the remaining collateral after deducting losses and fees have been sent back to your account`;
  return (
    <Tooltip
      position="left-top"
      handle={label}
...
export default function TradeHistory(props) {
  const { account, infoTokens, getTokenInfo, chainId, shouldShowPaginationButtons } = props;
  ...
  const getMsg = useCallback(
    ...
    [getTokenInfo, infoTokens, liquidationsDataMap,]
  );
  ...
  return (
    <div className="TradeHistory">
      {tradesWithMessages.length === 0 && (
        <div className="TradeHistory-row App-box App-box-none-radius">
