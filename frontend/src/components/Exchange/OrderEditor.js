...
export default function OrderEditor(props) {
  ...
  const fromTokenInfo = order.type === SWAP ? getTokenInfo(infoTokens, order.path[0]) : null;
  const toTokenInfo =
    order.type === SWAP
      ? getTokenInfo(infoTokens, order.path[order.path.length - 1])
      : null;
      ...
      >
        <div className="Exchange-swap-section">
          <div className="Exchange-swap-section-top">
            ...
            <div
              ...
              onClick={() => {
                setTriggerPriceValue(formatAmountFree(indexTokenMarkPrice, USD_DECIMALS, 4));
              }}
            >
              <Trans>Mark: {formatAmount(indexTokenMarkPrice, USD_DECIMALS, 4)}</Trans>
            </div>
          </div>
          ...
          {triggerPrice && !triggerPrice.eq(order.triggerPrice) ? (
            <>
              <span className="muted">
                {triggerPricePrefix} {formatAmount(order.triggerPrice, USD_DECIMALS, 4, true)}
              </span>
              &nbsp;
              <BsArrowRight />
              &nbsp;
              {triggerPricePrefix} {formatAmount(triggerPrice, USD_DECIMALS, 4, true)}
            </>
          ) : (
            <span>
              {triggerPricePrefix} {formatAmount(order.triggerPrice, USD_DECIMALS, 4, true)}
        ...
        {liquidationPrice && (
          <div className="Exchange-info-row">
            ...
            <div className="align-right">{`$${formatAmount(liquidationPrice, USD_DECIMALS, 4, true)}`}</div>
          </div>
        )}
