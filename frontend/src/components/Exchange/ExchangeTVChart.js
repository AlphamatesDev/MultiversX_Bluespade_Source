...
import { getPriceDecimals, getToken, getTokens } from "config/tokens";
...
export default function ExchangeTVChart(props) {
  ...
  const symbol = chartToken ? chartToken.symbol : undefined;
    ...
    if (currentSeries && savedShouldShowPositionLines) {
        ...
        currentOrders.forEach((order) => {
          ...
          lines.push(
            currentSeries.createPriceLine({
              price: parseFloat(formatAmount(order.triggerPrice, USD_DECIMALS, indexToken.priceDecimals)),
              color,
      ...
      if (positions && positions.length > 0) {
        ...
        positions.forEach((position) => {
          ...
          lines.push(
            currentSeries.createPriceLine({
              price: parseFloat(formatAmount(position.averagePrice, USD_DECIMALS, position.indexToken.pricede)),
              color,
          ...
          lines.push(
            currentSeries.createPriceLine({
              price: parseFloat(formatAmount(liquidationPrice, USD_DECIMALS, getPriceDecimals(chainId, position.indexToken.symbol))),
              color,
  ...
  //     [`length-${String(candlestick.close.toFixed((symbol === "CRO" || symbol === "ADA" || symbol === "DOGE" || symbol === "MATIC" || symbol === "CRV") ? 6 : 4)).length}`]: true,
  //   });

  //   const toFixedNumbers = (symbol === "CRO" || symbol === "ADA" || symbol === "DOGE" || symbol === "MATIC" || symbol === "CRV") ? 6 : 4;
  ...
  if (deltaPrice && currentAveragePrice) {
    const average = parseFloat(formatAmount(currentAveragePrice, USD_DECIMALS, chartToken.priceDecimals));
        ...
        <div className="ExchangeChart-top-inner">
          ...
          <div>
            <div className="ExchangeChart-main-price">
              {chartToken.maxPrice && formatAmount(chartToken.maxPrice, USD_DECIMALS, chartToken.priceDecimals, true)}
            </div>
            <div className="ExchangeChart-info-label">
              ${chartToken.minPrice && formatAmount(chartToken.minPrice, USD_DECIMALS, chartToken.priceDecimals, true)}
            </div>
          </div>
          <div>
            <div className="ExchangeChart-info-label">
          ...
          <div className="ExchangeChart-additional-info">
            ...
            <div>
              {!high && "-"}
              {high && numberWithCommas(high.toFixed(chartToken.priceDecimals))}
            </div>
          </div>
          <div className="ExchangeChart-additional-info">
            <div className="ExchangeChart-info-label">
            ...
            <div>
              {!low && "-"}
              {low && numberWithCommas(low.toFixed(chartToken.priceDecimals))}
            </div>
          </div>
        </div>
      </div>
