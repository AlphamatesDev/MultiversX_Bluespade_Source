...
const getOrdersForPosition = (account, position, orders) => {
...
export default function PositionsList(props) {
  ...
  return (
    <div className="PositionsList">
        ...
        <div className="Exchange-list small">
          <div>
            {positions.length === 0 && positionsDataIsLoading && (
              <div className="Exchange-empty-positions-list-note App-card App-card-none-radius">
                <Trans>Loading...</Trans>
              </div>
            )}
            {positions.length === 0 && !positionsDataIsLoading && (
            ...
            {positions.map((position) => {
              const positionOrders = getOrdersForPosition(account, position, orders);
              // const liquidationPrice = getLiquidationPrice(position);
              const { stopLossAmount, stopLossPrice } = getStopLossStatus(position);
              const { takeProfitAmount, takeProfitPrice } = getTakeProfitStatus(position);
                ...
                <div key={position.key} className="App-card">
                    ...
                    <div className="App-card-row">
                      ...
                      <div>
                        ...
                        {positionOrders.map((order) => {
                          const orderText = () => (
                            <>
                              {order.triggerAboveThreshold ? ">" : "<"} {formatAmount(order.triggerPrice, 30, 4, true)}:
                  ...
                  <div className="App-card-content">
                    ...
                    <div className="App-card-row">
                      ...
                      <div className="position-list-collateral">
                        <Tooltip
                          handle={`-${position.slPercent}%`}
                          position="right-bottom"
                          handleClassName={cx("plain", "negative")}
                          renderContent={() => {
                            return (
                              <>
                                <StatsTooltipRow
                                  label={t`Stop Loss Price`}
                                  value={formatAmount(stopLossPrice, USD_DECIMALS, 2, true)}
                                />
                                <StatsTooltipRow
                                  label={t`Stop Loss Amount`}
                                  value={formatAmount(stopLossAmount, USD_DECIMALS, 2, true)}
                                />
                              </>
                            );
                          }}
                    ...
                    <div className="App-card-row">
                      ...
                      <div className="position-list-collateral">
                        <Tooltip
                          handle={`+${position.tpPercent}%`}
                          position="right-bottom"
                          handleClassName={cx("plain", "positive")}
                          renderContent={() => {
                            return (
                              <>
                                <StatsTooltipRow
                                  label={t`Take Profit Price`}
                                  value={formatAmount(takeProfitPrice, USD_DECIMALS, 2, true)}
                                />
                                <StatsTooltipRow
                                  label={t`Take Profit Amount`}
                                  value={formatAmount(takeProfitAmount, USD_DECIMALS, 2, true)}
                                />
                              </>
                            );
                          }}
      ...
      <table className="Exchange-list large App-box App-box-none-radius">
        <tbody>
          ...
          {positions.map((position) => {
            ...
            return (
              <tr key={position.key}>
                              ...
                              {positionOrders.map((order) => {
                                return (
                                  ...
                                  >
                                    ...
                                    {formatAmount(order.triggerPrice, 30, 4, true)}:
                                    {order.type === INCREASE ? " +" : " -"}${formatAmount(order.sizeDelta, 30, 2, true)}
                                    {order.error && (
                                      <>
                ...
                <td className="clickable" onClick={() => onPositionClick(position)}>
                  <Tooltip
                    handle={`$${formatAmount(position.markPrice, USD_DECIMALS, 4, true)}`}
                    position="left-bottom"
