...
export default function PositionEditor(props) {
  ...
  if (position) {
    ...
    if (fromAmount) {
      ...
      nextLiquidationPrice = getLiquidationPrice({
        ...
        collateralDelta,
        increaseCollateral: isDeposit,
        isSuperTrader: position.isSuperTrader
      });
  ...
  const getError = () => {
    ...
    if (nextLeverageExcludingPnl && nextLeverageExcludingPnl.gt(1250 * BASIS_POINTS_DIVISOR)) {
  ...
  const depositCollateral = async () => {
    ...
    callContract(chainId, contract, method, params, {
      ...
      successMsg: t`Requested deposit of ${formatAmount(fromAmount, position.collateralToken.decimals, 4)} ${position.collateralToken.symbol
  ...
  const withdrawCollateral = async () => {
    ...
    callContract(chainId, contract, method, params, {
      ...
      successMsg: t`Requested withdrawal of ${formatAmount(fromAmount, USD_DECIMALS, 2)} USD from ${position.indexToken.symbol
        } ${longOrShortText}.`,
                  ...
                  <div className="Exchange-info-row">
                    ...
                    <div className="align-right">
                      {!nextLiquidationPrice && (
                        <div>
                          {!fromAmount && `$${formatAmount(liquidationPrice, USD_DECIMALS, 4, true)}`}
                          {fromAmount && "-"}
                        </div>
                      )}
                      {nextLiquidationPrice && (
