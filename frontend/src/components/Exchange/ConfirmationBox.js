...
import { /*getCollateralToken,*/ getToken } from "config/tokens";
import { Plural, t, Trans } from "@lingui/macro";
// import ExternalLink from "components/ExternalLink/ExternalLink";

const HIGH_SPREAD_THRESHOLD = expandDecimals(1, USD_DECIMALS).div(100); // 1%;

function getSpread(fromTokenInfo, toTokenInfo, isLong) {
...
export default function ConfirmationBox(props) {
  ...
  const existingOrder = useMemo(() => {
    ...
    for (const order of orders) {
      ...
      const sameToken = order.indexToken === toToken.address;
      if (order.isLong === isLong && sameToken) {
        return order;
      }
    }
  }, [orders, isLong, toToken.address]);
  ...
  const spread = getSpread(fromTokenInfo, toTokenInfo, isLong);
  ...
  }, [feeBps, collateralTokenAddress, chainId, fromToken.symbol, orderOption]);
  ...
  }, [decreaseOrdersThatWillBeExecuted, chainId, library, pendingTxns, setPendingTxns, isLong]);
  ...
  const renderMain = useCallback(() => {
    return (
      <div className="Confirmation-box-main">
        <span>
          <Trans>Pay</Trans>&nbsp;{formatAmount(fromAmount, fromToken.decimals, 0, true)} {fromToken.symbol} ($
          {formatAmount(fromUsdMin, USD_DECIMALS, 0, true)})
        </span>
        ...
        <div>
          {formatAmount(toAmount, fromToken.decimals, 0, true)} {fromToken.symbol} ($
          {formatAmount(toUsdMax, USD_DECIMALS, 0, true)})
        </div>
      </div>
    );
  ...
  //       {formatAmount(executionFee, 18, 4)} {getCollateralToken(chainId).symbol}
  //     </ExchangeInfoRow>
  //   );
    ...
    return (
      <>
        <div>
          ...
          <ExchangeInfoRow label={t`Collateral In`} value={fromTokenInfo.symbol} />
          ...
          <ExchangeInfoRow label={t`Stop Loss`}>
            {hasExistingPosition && toAmount && toAmount.gt(0) && (
              <div className="inline-block muted">
            ...
            {toAmount && displayLiquidationPrice && `$${formatAmount(displayLiquidationPrice, USD_DECIMALS, 4, true)}`}
