...
import { getHomeUrl/**, getRootShareApiUrl */, USD_DECIMALS } from "lib/legacy";
...
function PositionShare({ setIsPositionShareModalOpen, isPositionShareModalOpen, positionToShare, account, chainId }) {
  ...
  //   `Latest $${positionToShare?.indexToken?.symbol} trade on @bluespade`,
  ...
  function handleCopy() {
    ...
    const url = "http://app.bluespade.xyz/#/trade/?ref=" + userAffiliateCode.code;
    copyToClipboard(url);
  ...
  return (
    <div className="relative">
      <div ref={positionRef} className="position-share" style={{ backgroundImage: `url(${sharePositionBgImg})` }}>
        <img className="logo" src={gmxLogo} alt="BLU Logo" />
        <ul className="info">
