...
function shouldSwap(collateralToken, receiveToken) {
  const isSameToken = collateralToken.address === receiveToken.address;
  return !isSameToken;
}
...
export default function PositionSeller(props) {
  ...
  // const ORDER_OPTIONS =  [MARKET];//[MARKET, STOP];
  ...
  const existingOrders = useMemo(() => {
    ...
    for (const order of orders) {
      ...
      const sameToken = order.indexToken === position.indexToken.address;
      if (order.isLong === position.isLong && sameToken) {
        ret.push(order);
      }
    }
    return ret;
  }, [position, orders, triggerPriceUsd, orderOption]);
  ...
  let executionFeeUsd = getUsd(executionFee, position.collateralToken.address, false, infoTokens) || bigNumberify(0);
  ...
  if (position) {
    ...
    if (fromAmount) {
      ...
      positionFee = getMarginFee(fromAmount, isSuperTrader);
    }
  ...
  const getError = () => {
    ...
    if (orderOption === STOP) {
      ...
      if (position.isLong && triggerPriceUsd.lte(liquidationPrice)) {
        return t`Price below Stop Loss`;
      }
      if (!position.isLong && triggerPriceUsd.gte(liquidationPrice)) {
        return t`Price above Stop Loss`;
      }
    ...
    if (nextLeverage && nextLeverage.gt(30.5 * BASIS_POINTS_DIVISOR)) {
      return t`Max leverage: 1250.5xt`;
    }
  ...
  const onClickPrimary = async () => {
    ...
    const indexTokenAddress = position.indexToken.address;
        ...
        >
            ...
            <div className="Exchange-swap-section">
              <div className="Exchange-swap-section-top">
                ...
                <div
                  ...
                  onClick={() => {
                    setTriggerPriceValue(formatAmountFree(position.markPrice, USD_DECIMALS, 4));
                  }}
                >
                  <Trans>Mark: {formatAmount(position.markPrice, USD_DECIMALS, 4, true)}</Trans>
                </div>
              </div>
              <div className="Exchange-swap-section-bottom">
                <div className="Exchange-swap-input-container">
          ...
          <div className="PositionEditor-info-box">
            ...
            {orderOption === STOP && (
              <div className="Exchange-info-row">
                ...
                <div className="align-right">
                  {!triggerPriceUsd && "-"}
                  {triggerPriceUsd && `${triggerPricePrefix} ${formatAmount(triggerPriceUsd, USD_DECIMALS, 4, true)}`}
                </div>
              </div>
            )}
            <div className="Exchange-info-row top-line">
              ...
              <div className="align-right">${formatAmount(position.markPrice, USD_DECIMALS, 4, true)}</div>
            </div>
            <div className="Exchange-info-row">
              <div className="Exchange-info-label">
                <Trans>Entry Price</Trans>
              </div>
              <div className="align-right">${formatAmount(position.averagePrice, USD_DECIMALS, 4, true)}</div>
            </div>
            <div className="Exchange-info-row">
              <div className="Exchange-info-label">
                    ...
                    {nextLiquidationPrice && !nextLiquidationPrice.eq(liquidationPrice) && (
                      <div>
                        <div className="inline-block muted">
                          ${formatAmount(liquidationPrice, USD_DECIMALS, 4, true)}
                          <BsArrowRight className="transition-arrow" />
                        </div>
                        ${formatAmount(nextLiquidationPrice, USD_DECIMALS, 4, true)}
                      </div>
                    )}
                  </div>
                )}
