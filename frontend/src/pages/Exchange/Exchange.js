...
import { useAllOrders } from "domain/legacy";
...
import SuperTrader from "abis/SuperTrader.json";
...
export function getPositions(
  ...
  for (let i = 0; i < collateralTokens.length; i++) {
    ...
    const position = {
      ...
      slPercent: positionData[i * propsLength + 9],
      tpPercent: positionData[i * propsLength + 10],
    ...
    position.closingFee = position.size.mul(isSuperTrader ? MARGIN_FEE_BASIS_POINTS_SUPER : MARGIN_FEE_BASIS_POINTS_NORMAL).div(BASIS_POINTS_DIVISOR);
    position.positionFee = position.size.mul(isSuperTrader ? MARGIN_FEE_BASIS_POINTS_SUPER : MARGIN_FEE_BASIS_POINTS_NORMAL).mul(2).div(BASIS_POINTS_DIVISOR);
...
export function getPositionQuery(tokens, collateralTokenAddress) {
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    collateralTokens.push(collateralTokenAddress);
...
export const Exchange = forwardRef((props, ref) => {
  ...
  const defaultIndexTokenAddress = getContract(chainId, "DEFAUT_INDEX_TOKEN")
  const vaultAddress = getContract(chainId, "Vault");
  const positionRouterAddress = getContract(chainId, "PositionRouter");
  const readerAddress = getContract(chainId, "Reader");
  const usdgAddress = getContract(chainId, "USDG");
  const superTraderAddress = getContract(chainId, "SuperTrader");

  const whitelistedTokens = getWhitelistedTokens(chainId);
  const whitelistedTokenAddresses = whitelistedTokens.map((token) => token.address);

  const positionQuery = getPositionQuery(whitelistedTokens, collateralTokenAddress);

  const defaultCollateralSymbol = getConstant(chainId, "defaultCollateralSymbol");
  const defaultTokenSelection = useMemo(
    () => ({
      [SWAP]: {
        from: collateralTokenAddress,
        to: defaultIndexTokenAddress,
      },
      [LONG]: {
        from: collateralTokenAddress,
        to: defaultIndexTokenAddress,
    ...
    [chainId, defaultCollateralSymbol, collateralTokenAddress, defaultIndexTokenAddress]
  );
  ...
  const fromTokenAddress = (tokens.filter((token) => token.isCollateralToken))[0].address;
  ...
  const { data: fundingRateInfo } = useSWR([active, chainId, readerAddress, "getFundingRates"], {
    fetcher: contractFetcher(library, Reader, [vaultAddress, whitelistedTokenAddresses]),
  });
  ...
  const { data: isSuperTrader } = useSWR([`Exchange:isSuperTrader:${active}`, chainId, superTraderAddress, "isSuperTrader", account], {
    fetcher: contractFetcher(library, SuperTrader)
  })
  ...
  useEffect(() => {
    ...
    let currentTokenPriceStr = formatAmount(selectedToken.maxPrice, USD_DECIMALS, 4, true);
  ...
  useImperativeHandle(ref, () => ({
    ...
    onIncreasePosition(key, account, collateralToken, indexToken, collateralDelta, sizeDelta, isLong, price, fee, e) {
      ...
      const tokenSymbol = indexTokenItem.symbol;
  ...
  const orders = useAllOrders(chainId, library, account);
  ...
  return (
    <div className="Exchange page-layout">
      ...
      <div className="Exchange-content">
        <div className="Exchange-gradient-bg-dark"></div>
