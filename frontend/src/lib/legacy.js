...
export const MAX_LEVERAGE = 1750 * 10000;
export const MAX_ALLOWED_LEVERAGE = 1250 * 10000;
...
export const MARGIN_FEE_BASIS_POINTS_NORMAL = 10;
export const MARGIN_FEE_BASIS_POINTS_SUPER = 5;
...
export const LEVERAGE_ORDER_OPTIONS = [MARKET, LIMIT];//[MARKET, LIMIT, STOP];
...
export function getMarginFee(sizeDelta, isSuperTrader) {
  if (!sizeDelta) {
    return bigNumberify(0);
  }
  const afterFeeUsd = sizeDelta.mul(BASIS_POINTS_DIVISOR - (isSuperTrader ? MARGIN_FEE_BASIS_POINTS_SUPER : MARGIN_FEE_BASIS_POINTS_NORMAL)).div(BASIS_POINTS_DIVISOR);
...
export function getLeverage({
  ...
  remainingCollateral = sizeDelta
    ? remainingCollateral.mul(BASIS_POINTS_DIVISOR - isSuperTrader ? MARGIN_FEE_BASIS_POINTS_SUPER : MARGIN_FEE_BASIS_POINTS_NORMAL).div(BASIS_POINTS_DIVISOR)
    : remainingCollateral;
...
export function getLiquidationPrice(data) {
  ...
  let positionFee = getMarginFee(size, isSuperTrader).add(LIQUIDATION_FEE);
  if (entryFundingRate && cumulativeFundingRate) {
    const fundingFee = size.mul(cumulativeFundingRate.sub(entryFundingRate)).div(FUNDING_RATE_PRECISION);
...
export function getStopLossStatus(data) {
  let {
    isLong,
    size,
    collateral,
  ...
  const stopLossAmount = collateral.mul(slPercent).div(100);

  const liquidationPriceStopLossAmount = getLiquidationPriceFromDelta({
    liquidationAmount: collateral.sub(stopLossAmount),
    size: nextSize,
    collateral: remainingCollateral,
    averagePrice,
    isLong,
  ...
  return {
    ...
    stopLossPrice: liquidationPriceStopLossAmount,
  }
}

export function getTakeProfitStatus(data) {
  let {
    isLong,
    size,
    collateral,
  ...
  const takeProfitAmount = collateral.mul(100 + parseInt(tpPercent)).div(100);

  const liquidationPriceTakeProfitAmount = getLiquidationPriceFromDelta({
    liquidationAmount: takeProfitAmount,
    size: nextSize,
    collateral: remainingCollateral,
    averagePrice,
    isLong,
  ...
  return {
    ...
    takeProfitPrice: liquidationPriceTakeProfitAmount,
  }
}

export function getPositionKey(account, collateralTokenAddress, indexTokenAddress, isLong) {
  const tokenAddress0 = collateralTokenAddress;
  const tokenAddress1 = indexTokenAddress;
...
export function getProcessedData(
  ...
  data.feeGmxTrackerRewardsUsd = stakingData.feeGmxTracker.claimable.mul(collateralTokenPrice).div(expandDecimals(1, 18));
  ...
  data.feeGlpTrackerRewardsUsd = stakingData.feeGlpTracker.claimable.mul(collateralTokenPrice).div(expandDecimals(1, 18));
...
export function getTradePageUrl() {
  ...
  return "https://app.bluespade.xyz/#/trade";
}
