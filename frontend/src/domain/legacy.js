...
import { POLYGON, CRONOS, SKALE, getHighExecutionFee } from "config/chains";
import { DECREASE, getOrderKey, INCREASE, SWAP, USD_DECIMALS } from "lib/legacy";

import { groupBy } from "lodash";
import { UI_VERSION } from "config/ui";
import { getServerBaseUrl, getServerUrl } from "config/backend";
import { getGmxGraphClient, /*nissohGraphClient, avalancheGraphClientForTrades,*/ polygonGraphClientForTrades, cronosGraphClientForTrades, skaleGraphClientForTrades, getGmxGraphClientByNewCreate } from "lib/subgraph/clients";
...
const FIRST_DATE_TS_POLYGON = parseInt(+(new Date(2023, 5, 19)) / 1000) + 62899
...
export function usePositionStatesCronos(chainId) {
  const [res, setRes] = useState();
  useEffect(() => {
    const query = gql(`
    {
    ...
    if (!cronosGraphClientForTrades) {
      return;
    }

    cronosGraphClientForTrades
      .query({ query })
      .then((res) => {
        setRes(res);
      })
  ...
  const tradeVolueDataMap = useMemo(() => {
    ...
    tradeData.createIncreasePositions.forEach((item) => {
      if(item.isLong) {
        ...
        superArray.totalLongPositionSizes = superArray.totalLongPositionSizes.add(bigNumberify(item.sizeDelta).div(bigNumberify("1000000000000")));
      } else {
        superArray.totalShortPositionCollaterals = superArray.totalShortPositionCollaterals.add(bigNumberify(item.amountIn));
        superArray.totalShortPositionSizes = superArray.totalShortPositionSizes.add(bigNumberify(item.sizeDelta).div(bigNumberify("1000000000000")));
      }
...
export function usePositionStatesPolygon(chainId) {
  const [res, setRes] = useState();
  useEffect(() => {
    const query = gql(`
    {
...
export function useStakedGmxSupply(library, active) {
  const gmxAddressCronos = getContract(CRONOS, "GMX");
  const stakedGmxTrackerAddressCronos = getContract(CRONOS, "StakedGmxTracker");

  const { data: cronosData, mutate: cronosMutate } = useSWR(
    [`StakeV2:stakedGmxSupply:${active}`, CRONOS, gmxAddressCronos, "balanceOf", stakedGmxTrackerAddressCronos],
    {
      fetcher: contractFetcher(undefined, Token),
    }
  );
  
  const gmxAddressPolygon = getContract(POLYGON, "GMX");
  const stakedGmxTrackerAddressPolygon = getContract(POLYGON, "StakedGmxTracker");

  const { data: polygonData, mutate: polygonMutate } = useSWR(
    [`StakeV2:stakedGmxSupply:${active}`, POLYGON, gmxAddressPolygon, "balanceOf", stakedGmxTrackerAddressPolygon],
    {
      fetcher: contractFetcher(undefined, Token),
    }
  );
  ...
  let data;
  if (cronosData && polygonData && skaleData) {
    data = cronosData.add(polygonData).add(skaleData);
  }
...
export function useHasOutdatedUi() {
  const url = getServerUrl(CRONOS, "/ui_version");
...
export function useGmxPrice(chainId, libraries, active) {
  const { data: gmxPriceFromCronos, mutate: mutateFromCronos } = useGmxPriceFromCronos();
  const { data: gmxPriceFromPolygon, mutate: mutateFromPolygon } = useGmxPriceFromCronos();// useGmxPriceFromPolygon();
  const { data: gmxPriceFromSkale, mutate: mutateFromSkale } = useGmxPriceFromCronos();// useGmxPriceFromSkale();

  const gmxPrice = chainId === CRONOS ? gmxPriceFromCronos : (chainId === POLYGON ? gmxPriceFromPolygon : gmxPriceFromSkale);
  const mutate = useCallback(() => {
    mutateFromCronos();
    mutateFromPolygon();
    mutateFromSkale();
  }, [mutateFromCronos, mutateFromPolygon, mutateFromSkale]);
...
export function useTotalGmxSupply() {
  ...
  const lockGmxContractAddressCronos = getContract(CRONOS, "LOCK");
  const { data: lockedGmxBalanceCronos } = useSWR(
    [
      `StakeV2:lockedGmxBalance:${CRONOS}`,
      CRONOS,
      getContract(CRONOS, "GMX"),
      "balanceOf",
      lockGmxContractAddressCronos,
    ],
    {
      fetcher: contractFetcher(undefined, Token),
    }
  ...
  const { data: gmxSupplyCronosReal } = useSWR([`StakeV2:totalSupply:${CRONOS}`, CRONOS, getContract(CRONOS, "GMX"), "totalSupply"], {
    fetcher: contractFetcher(undefined, Token),
  });

  let gmxSupplyCronos = bigNumberify(0);

  if (gmxSupplyCronosReal && lockedGmxBalanceCronos)
    gmxSupplyCronos = bigNumberify(gmxSupplyCronosReal).sub(lockedGmxBalanceCronos);

  const { data: gmxSupplyPolygon } = useSWR([`StakeV2:totalSupply:${POLYGON}`, POLYGON, getContract(POLYGON, "GMX"), "totalSupply"], {
    fetcher: contractFetcher(undefined, Token),
  });
  ...
  let gmxSupply = bigNumberify(0);
  if (gmxSupplyCronos && gmxSupplyPolygon && gmxSupplySkale) {
    // let total = bigNumberify(gmxSupplyCronos).add(gmxSupplyPolygon).add(gmxSupplySkale);
    let total = gmxSupplySkale;
    gmxSupply = total;
  }

  return {
    cronos: gmxSupplyCronos,
    polygon: gmxSupplyPolygon,
...
export function useTotalGmxStaked() {
  ...
  const { data: stakedGmxSupplyCronos, mutate: updateStakedGmxSupplyCronos } = useSWR(
    [
      `StakeV2:stakedGmxSupply:${CRONOS}`,
      CRONOS,
      getContract(CRONOS, "GMX"),
      "balanceOf",
      stakedGmxTrackerAddressCronos,
    ],
    {
      fetcher: contractFetcher(undefined, Token),
    }
  ...
  const { data: stakedGmxSupplyPolygon, mutate: updateStakedGmxSupplyPolygon } = useSWR(
    [
      `StakeV2:stakedGmxSupply:${POLYGON}`,
      POLYGON,
      getContract(POLYGON, "GMX"),
      "balanceOf",
      stakedGmxTrackerAddressPolygon,
    ],
    {
      fetcher: contractFetcher(undefined, Token),
    }
  ...
  const mutate = useCallback(() => {
    updateStakedGmxSupplyCronos();
    updateStakedGmxSupplyPolygon();
    updateStakedGmxSupplySkale();
  }, [updateStakedGmxSupplyCronos, updateStakedGmxSupplyPolygon, updateStakedGmxSupplySkale]);

  if (stakedGmxSupplyCronos && stakedGmxSupplyPolygon && stakedGmxSupplySkale) {
    // let total = bigNumberify(stakedGmxSupplyCronos).add(stakedGmxSupplyPolygon).add(stakedGmxSupplySkale);
    let total = stakedGmxSupplySkale;
    totalStakedGmx.current = total;
  }

  return {
    cronos: stakedGmxSupplyCronos,
    polygon: stakedGmxSupplyPolygon,
...
export function useTotalGmxInLiquidity() {
  let poolAddressCronos = getContract(CRONOS, "TraderJoeGmxAvaxPool");
  let poolAddressPolygon = getContract(POLYGON, "TraderJoeGmxAvaxPool");
  let poolAddressSkale = getContract(SKALE, "TraderJoeGmxAvaxPool");
  let totalGMX = useRef(bigNumberify(0));

  const { data: gmxInLiquidityOnCronos, mutate: mutateGMXInLiquidityOnCronos } = useSWR(
    [`StakeV2:gmxInLiquidity:${CRONOS}`, CRONOS, getContract(CRONOS, "GMX"), "balanceOf", poolAddressCronos],
    {
      fetcher: contractFetcher(undefined, Token),
    }
  );

  const { data: gmxInLiquidityOnPolygon, mutate: mutateGMXInLiquidityOnPolygon } = useSWR(
    [`StakeV2:gmxInLiquidity:${POLYGON}`, POLYGON, getContract(POLYGON, "GMX"), "balanceOf", poolAddressPolygon],
    {
      fetcher: contractFetcher(undefined, Token),
    }
  );
  ...
  const mutate = useCallback(() => {
    mutateGMXInLiquidityOnCronos();
    mutateGMXInLiquidityOnPolygon();
    mutateGMXInLiquidityOnSkale();
  }, [mutateGMXInLiquidityOnCronos, mutateGMXInLiquidityOnPolygon, mutateGMXInLiquidityOnSkale]);

  if (gmxInLiquidityOnCronos && gmxInLiquidityOnPolygon && gmxInLiquidityOnSkale) {
    let total = bigNumberify(gmxInLiquidityOnCronos).add(gmxInLiquidityOnPolygon).add(gmxInLiquidityOnSkale);
    totalGMX.current = total;
  }
  return {
    cronos: gmxInLiquidityOnCronos,
    polygon: gmxInLiquidityOnPolygon,
...
//   const poolAddress = getContract(POLYGON, "TraderJoeGmxAvaxPool");

//   const { data, mutate: updateReserves } = useSWR(["TraderJoeGmxAvaxReserves", POLYGON, poolAddress, "getReserves"], {
...
//   const wmaticAddress = getTokenBySymbol(POLYGON, "WMATIC").address;
//   const { data: maticPrice, mutate: updateEthPrice } = useSWR(
//     [`StakeV2:ethPrice`, POLYGON, vaultAddress, "getMinPrice", wmaticAddress],
//     {
...
function useGmxPriceFromCronos() {
  const poolAddress = getContract(CRONOS, "TraderJoeGmxAvaxPool");
  const { data, mutate: updateReserves } = useSWR(["TraderJoeGmxAvaxReserves", CRONOS, poolAddress, "getReserves"], {
    fetcher: contractFetcher(undefined, UniswapV2Cro),
  });
  const { _reserve0: gmxReserve, _reserve1: croReserve } = data || {};
  const vaultAddress = getContract(CRONOS, "Vault");
  const croAddress = getTokenBySymbol(CRONOS, "WCRO").address;
  const { data: croPrice, mutate: updateCroPrice } = useSWR(
    [`StakeV2:croPrice`, CRONOS, vaultAddress, "getMinPrice", croAddress],
    {
      fetcher: contractFetcher(undefined, Vault),
    }
  );
  ...
  const PRECISION = bigNumberify(10).pow(18);
  let gmxPrice;
  if (croReserve && gmxReserve && croPrice) {
    gmxPrice = croReserve.mul(PRECISION).div(gmxReserve).mul(croPrice).div(PRECISION);
    // gmxPrice = bigNumberify(65).mul(bigNumberify(10).pow(27));
  }

  const mutate = useCallback(() => {
    updateReserves(undefined, true);
    updateCroPrice(undefined, true);
  }, [updateReserves, updateCroPrice]);
...
export function useFeesDataCronos({ from = FIRST_DATE_TS, to = NOW_TS } = {}) {
  ...
  useEffect(() => {
    const graphClient = getGmxGraphClient(CRONOS);
    if (!graphClient) {
      return;
    }
    graphClient.query({ query }).then(setRes).catch(console.warn);
...
export function useFeesDataPolygon({ from = FIRST_DATE_TS_POLYGON, to = NOW_TS } = {}) {
  const PROPS = 'margin liquidation swap mint burn'.split(' ')
  const query = gql(`{
    feeStats(
      first: 1000
...
export function useVolumeDataCronos({ from = FIRST_DATE_TS, to = NOW_TS } = {}) {
	const PROPS = 'margin liquidation swap mint burn'.split(' ')
...
export function useVolumeDataPolygon({ from = FIRST_DATE_TS_POLYGON, to = NOW_TS } = {}) {
	const PROPS = 'margin liquidation swap mint burn'.split(' ')
  // const timestampProp = chainName === "arbitrum" ? "id" : "timestamp"
  const timestampProp = "timestamp"
  const [graphData, setRes] = useState();
