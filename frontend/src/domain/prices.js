...
const FEED_ID_MAP = {
  ...
  CRO_USD: "0xb2ad164c008da54fceac79ef18c0a8fe2d935865",
};
...
export async function getChartPricesFromStats(chainId, symbol, period) {
  if (["WBTC", "WETH", "WAVAX", "WBNB"].includes(symbol)) {
    symbol = symbol.substr(1);
  } else if (symbol === "BTC.b") {
    symbol = "BTC";
  }

  // if(!symbol.includes("BTC") && !symbol.includes("ETH") && !symbol.includes("AVAX") && !symbol.includes("BNB"))
...
export function getChainlinkChartPricesFromGraph(tokenSymbol, period) {
  if (["WBTC", "WETH", "WCRO"].includes(tokenSymbol)) {
